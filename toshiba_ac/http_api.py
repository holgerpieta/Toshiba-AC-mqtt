...
class ToshibaAcHttpApi:
    BASE_URL = 'mobileapi.toshibahomeaccontrols.com'
    ...
    async def get_devices_energy_consumption( self, ac_unique_ids, offset_days = None, total = True ):
        if offset_days is None:
            year = int(datetime.datetime.now().year)
            since = datetime.datetime(year, 1, 2).astimezone(datetime.timezone.utc)
            ...
            until = datetime.datetime.utcnow() - offset
            since = until - delta
            post = {
                'ACDeviceUniqueIdList': ac_unique_ids,
                'FromUtcTime': f'{since.isoformat()} UTC',
                'Timezone': 'UTC',
                'ToUtcTime': f'{until.isoformat()} UTC',
                'Type': 'EnergyDay'
            }

        logger.debug( f'Requesting energy: {post}')
        res = await self.request_api(self.AC_ENERGY_CONSUMPTION_PATH, post=post)
        logger.debug( f'Got energy response: {res}')
        ret = {}

        for ac in res:
            logger.debug( f'Energy object: {ac}')
            try:
                if ac['EnergyConsumption']:
                    logger.debug( f"Energy list: {ac['EnergyConsumption']}")
                    if total:
                        consumption = sum(int(consumption['Energy']) for consumption in ac['EnergyConsumption'])
                        logger.debug( f'Energy sum: {consumption}')
                        ret[ac['ACDeviceUniqueId']] = ToshibaAcDeviceEnergyConsumption(consumption, since)
                    else:
                        ret[ac['ACDeviceUniqueId']] = ac['EnergyConsumption']
                else:
                    logger.debug( f'No energy')
            except (KeyError, ValueError):
                logger.warning( 'Error in energy: %s' % ac['ACDeviceUniqueId'] )
                pass
